...
export default function EditPackage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleImageUpload = async (
    ...
    if (field === "cardImage" || field === "heroImage") {
      ...
      setFormData((prev) => (prev ? { ...prev, mapImage: url } : prev));
    }
  };
  ...
  const addHighlight = () => {
    if (highlightInput.trim() && formData) {
      setFormData((prev) => {
        if (!prev) return prev;
        return { ...prev, highlights: [...prev.highlights, highlightInput.trim()] };
      });
      setHighlightInput("");
    }
  };
  ...
  const addQuickInfo = () => {
    if (quickInfoInput.trim() && formData) {
      setFormData((prev) => {
        if (!prev) return prev;
        return { ...prev, quickInfo: [...prev.quickInfo, quickInfoInput.trim()] };
      });
      setQuickInfoInput("");
    }
  };
  ...
  const addItineraryDay = (optionIndex: number) => {
    setFormData((prev) => {
      ...
      const updatedOptions = [...prev.options];
      updatedOptions[optionIndex] = { ...opt, itineraryDays: [...currentDays, newDay] };
      return { ...prev, options: updatedOptions };
    });
  };
  ...
  const updateItineraryDay = (
    ...
    setFormData((prev) => {
      ...
      const updatedOptions = [...prev.options];
      updatedOptions[optionIndex] = { ...opt, itineraryDays: updatedDays };
      return { ...prev, options: updatedOptions };
    });
  };
  ...
  const addPriceTier = (optionIndex: number) => {
    setFormData((prev) => {
      ...
      const updatedOptions = [...prev.options];
      updatedOptions[optionIndex] = { ...opt, priceTiers: [...tiers, { minPax: 1, maxPax: 1, pricePerPerson: 0 }] };
      return { ...prev, options: updatedOptions };
    });
  };
  ...
  const updatePriceTier = (
    ...
    setFormData((prev) => {
      ...
      if (tiers[tierIndex]) {
        ...
        const updatedOptions = [...prev.options];
        updatedOptions[optionIndex] = { ...opt, priceTiers: tiers };
        return { ...prev, options: updatedOptions };
      }
      return prev;
    });
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="border rounded-lg p-4 bg-gray-50">
                  ...
                  <input
                    ...
                    accept="image/*"
                    className="block w-full text-sm text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-sm file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                    onChange={(e) => handleImageUpload(e, "mapImage")}
                  />
                  {formData.mapImage && (
                    <div className="relative mt-3 aspect-[4/3] w-full rounded-lg overflow-hidden border bg-white">
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData((prev) => (prev ? { ...prev, mapImage: "" } : prev))}
                        className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 shadow-sm"
                      >
                        <Trash2 className="w-3 h-3" />
                      </button>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  />
                  <button type="button" onClick={addHighlight} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
                    Add
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {formData.highlights.map((h, i) => (
                    <span key={i} className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                      {h}
                      <button type="button" onClick={() => removeHighlight(i)} className="text-red-600 hover:text-red-800 ml-1">
                        ×
                      </button>
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    placeholder="e.g., Best Time: June-Oct"
                  />
                  <button type="button" onClick={addQuickInfo} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
                    Add
                  </button>
                </div>
                    ...
                    <div className="p-6 space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => document.getElementById(`mainImage-${idx}`)?.click()}
                          ...
                          {opt.mainImage && (
                            <div className="relative mt-3 w-48 h-32 rounded-lg overflow-hidden border">
                              ...
                              <button
                                type="button"
                                onClick={() => updateOption(idx, "mainImage", "")}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateOption(idx, "accommodation", { ...accommodation, title: e.target.value })}
                            />
                          </div>
                          <div className="md:col-span-2">
                            ...
                            <textarea
                              ...
                              onChange={(e) => updateOption(idx, "accommodation", { ...accommodation, description: e.target.value })}
                            />
                          </div>
                        </div>
                        <div className="mt-3">
                          ...
                          <button
                            type="button"
                            onClick={() => document.getElementById(`accImages-${idx}`)?.click()}
                            className="px-4 py-2 border border-gray-300 rounded-lg text-sm hover:bg-gray-50 flex items-center gap-1"
                          >
                      ...
                      <div className="border-t pt-4">
                        <div className="flex justify-between items-center mb-3">
                          ...
                          <button type="button" onClick={() => addItineraryDay(idx)} className="bg-indigo-600 text-white px-3 py-1 rounded-md text-sm">
                            + Add Day
                          </button>
                        </div>
                          ...
                          {itineraryDays.map((day, dIdx) => (
                            <div key={dIdx} className="border rounded-lg p-4 bg-gray-50">
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <button type="button" onClick={() => removeItineraryDay(idx, dIdx)} className="text-red-500 text-sm">
                                  Remove Day
                                </button>
                              </div>
                              <input
                      ...
                      <div className="border-t pt-4">
                        ...
                        {opt.priceType === "tiered" && (
                          <div>
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <button type="button" onClick={() => addPriceTier(idx)} className="text-xs bg-gray-200 px-2 py-1 rounded hover:bg-gray-300">
                                + Add Tier
                              </button>
                            </div>
                            <div className="space-y-2">
                              {(opt.priceTiers || []).map((tier, tIdx) => (
                                <div key={tIdx} className="flex flex-wrap gap-2 items-end">
                                  ...
                                  <button type="button" onClick={() => removePriceTier(idx, tIdx)} className="text-red-500 text-sm">
                                    Remove
                                  </button>
                                </div>
                              ))}
                        ...
                        <div className="mt-4">
                          ...
                          <textarea
                            ...
                            placeholder="Additional details shown after 'Read more'..."
                          />
                        </div>
                      </div>
                    </div>
          ...
          {/* Sticky Action Bar */}
          <div className="sticky bottom-4 flex justify-end gap-4 bg-white/90 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-gray-200">
            <button
              type="button"
              onClick={() => router.back()}
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-blue-700 hover:bg-blue-800 text-white px-8 py-2.5 rounded-lg font-semibold transition disabled:opacity-50 flex items-center gap-2"
            >
