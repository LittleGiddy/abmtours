...
const GoogleReviews = () => {
  ...
  const fetchGoogleReviews = async (): Promise<void> => {
    try {
      ...
      if (data.success) {
        ...
        if (data.reviews?.length === 0) {
          setError('No reviews yet. Be the first to leave a review!');
        }
      } else {
  ...
  return (
    <div className="w-full">
      ...
      {/* Reviews List - Display REAL reviews */}
      <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
        {reviews.length > 0 ? (
          ...
          <div className="text-center text-white py-8">
            <p className="mb-4">{error || 'No reviews yet'}</p>
            <button
              onClick={openGoogleReviewForm}
              className="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-xl hover:shadow-lg transition-all duration-300 font-semibold cursor-pointer"
            >
